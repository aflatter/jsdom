...
core.resourceLoader = {
  load: function(element, href, callback) {
    ...
    if (ownerImplementation.hasFeature('FetchExternalResources', element.tagName.toLowerCase())) {
      var url = this.resolve(element._ownerDocument, href);
      if (!url) {
        element.trigger("error", "Could not resolve URL for '" + href + "'");
        return;
      }
      if (url.hostname) {
        this.download(url, this.enqueue(element, callback, URL.format(url)));
      }
      else {
        this.readFile(url, this.enqueue(element, callback, url.pathname));
      }
    }
  },
  ...
  resolve: function(document, href) {
    if (href.match(/^\w+:\/\//)) return URL.parse(href);
    ...
    var url = URL.resolveObject(baseUrl, href);
  ...
  readFile: function(url, callback) {
    fs.readFile(url.pathname, 'utf8', callback);
  }
};
